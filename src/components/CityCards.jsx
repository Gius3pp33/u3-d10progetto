...
const CityCard = ({ city,handleSearch }) => {
  ...
  return (
    <Col sm={6} md={4} lg={3} xl={2} className="mb-4">
      <Card className="city-card" onClick={() => handleSearch(city)} style={{ borderRadius: '8px', boxShadow: '0px 0px 10px rgba(0, 0, 0, 0.1)' }}>
        ...
        <Carousel.Item key={i}>
          <Row className="justify-content-around">
            {cities.slice(i, i + itemsPerSlide).map((city, index) => (
              <CityCard key={index} city={city} handleSearch={handleSearch} />
            ))}
          </Row>
        </Carousel.Item>
      );
